...
$WSPACE_BASE		= $ENV{APPRIS_PROGRAMS_TMP_DIR}.'/'.$cfg->val('APPRIS_PIPELINE', 'workspace').'/'.$cfg->val('SPADE_VARS', 'name').'/';
			...
			foreach my $alignment_report2 (@{$pfam_report2->{'domains'}})
			{
				...
				if ( $index_domain != -1 )
				{
					...
					{ # first domain found that has a better e_value
						...
						$alignment_report{'alignment_end'} = $index_domain + length($seq_domain2);
			...
			if ( scalar(@value_list) > 12 )
			{
				...
				if($e_value < $PROG_EVALUE)
				{
					...
					{							
						...
						# Get raw sequence						
						if ( $value_line =~ /#SEQ\s*([^\n]*)/ ) {
							$alignment_report->{'sequence'} = uc($1); # uppercase for gap alignment
						}					
						...
						my ($num_gaps) = &$get_gaps($value_line);
						...
						# Get type of domain from hmm length
						my (@D_SCORE) = ( 4, 7, 12 );
						my (@N_GAPS);
						if ( $hmm_length <= 100 ) {
							@N_GAPS = ( 3, 5, 7 );			
						}
						elsif ( $hmm_length <= 200 ) {
							@N_GAPS = ( 3, 6, 9 );			
						}
						else {
							@N_GAPS = ( 3, 7, 11 );			
						...
						if ( ($domain_score <= $D_SCORE[0]) and ($num_gaps <= $N_GAPS[0]) )
						{
							$alignment_report->{'type_domain'} = 'domain';
							$num_domains++;
						}
						elsif ( ($domain_score <= $D_SCORE[1]) and ($num_gaps <= $N_GAPS[1]) )
						{
							$alignment_report->{'type_domain'} = 'domain_possibly_damaged';
							$num_possibly_damaged_domains++;
						}
						elsif ( ($domain_score <= $D_SCORE[2]) and ($num_gaps <= $N_GAPS[2]) )
						{
...
sub _get_record_annotations($)
{
	...
	$output_content .= " *   predicts the presence of protein domains thanks to Pfamscan agent\n";
	$output_content .= " *   ftp://ftp.sanger.ac.uk/pub/databases/Pfam/Agents/README .\n";
	$output_content .= " * \n";

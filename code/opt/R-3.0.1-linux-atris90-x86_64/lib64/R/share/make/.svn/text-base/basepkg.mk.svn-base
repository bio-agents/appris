...
mkR1:
	...
	@(f=$${TMPDIR:-/tmp}/R$$$$; \
	  ...
	  $(SHELL) $(top_srcdir)/agents/move-if-change "$${f}" all.R)
...
mkRbase:
	...
	@(f=$${TMPDIR:-/tmp}/R$$$$; \
	  ...
	  rm -f "$${f}"; \
	  $(SHELL) $(top_srcdir)/agents/move-if-change "$${f2}" all.R)
...
mkdesc:
	@if test -f DESCRIPTION; then \
	  $(ECHO) "agents:::.install_package_description('.', '$(top_builddir)/library/${pkg}')" | \
...
mkdemos:
	@$(ECHO) "agents:::.install_package_demos('$(srcdir)', '$(top_builddir)/library/$(pkg)')" | \
	  R_DEFAULT_PACKAGES=NULL $(R_EXE) > /dev/null

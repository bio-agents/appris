...
plot.design( ergoSagent )   # produces Figure 1.6
contrasts( ergoSagent$Type )
ergoSagent1 <- ergoSagent[ ergoSagent$Subject == "1", ]
model.matrix( effort ~ Type, ergoSagent1 )   # X matrix for Subject 1
fm1Sagent <-
  lme(effort ~ Type, data = ergoSagent, random = ~ 1 | Subject)
summary( fm1Sagent )
anova( fm1Sagent )
...
summary( fm2Sagent )
anova( fm2Sagent )
model.matrix( effort ~ Type - 1, ergoSagent1 )
fm3Sagent <-
 lme(effort ~ Type - 1, data = ergoSagent, random = ~ 1 | Subject)
summary( fm3Sagent )
anova( fm3Sagent )
intervals( fm1Sagent )
plot( fm1Sagent,   # produces Figure 1.8
...
fm4Sagent <- lme( effort ~ Type, ergoSagent, ~ 1 | Subject/Type )
if (interactive()) intervals( fm4Sagent )
(fm1Sagent$sigma)^2
(fm4Sagent$sigma)^2 + 0.79621^2

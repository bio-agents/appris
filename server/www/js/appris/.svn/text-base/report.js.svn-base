...
function initPanelMenu(sPanel) {
	// Init configuration for (Menu report: Show,Hide, and Export agents)
	...
	$('#'+sPanel+' .menu .agent').css({ 'text-decoration': 'none', cursor: 'default' });
	...
	$('#'+sPanel+' .menu .show').click(function (eval){
		...
		if ($('#' + sParentId + ' .report').css('display') == 'none') {			
			...
			$('#' + sParentId + ' .menu .agent').css({ 'text-decoration': 'underline', cursor: 'pointer' });
			$('#' + sParentId + ' .menu .agent').bind('click',openAgentData);
	...
	$('#'+sPanel+' .menu .hide').click(function (eval){
		...
		if ($('#' + sParentId + ' .report').css('display') == 'block') {
			...
			$('#' + sParentId + ' .menu .agent').css({ 'text-decoration': 'none', cursor: 'default' });
			$('#' + sParentId + ' .menu .agent').unbind('click');
...
function closeExportData() {
	...
	$(this).parents('.agent_menu ul').fadeOut(300, function(){
			$(this).parent().parent().css({display: ''});
			$(this).remove();
	});
}
			...
			switch (sPanelType) {
				...
				case 'sequence_panel':					
					...
					$('table.sequence_panel input.sequence_id:checked').each(function() {
						var sVal = $(this).val();
						sSpecie = sVal.match(/^[^\_]*\_[^\_]*/);
						var sId = sVal.match(/[^\_]*$/); 
						sIdList += sId+',';
					});
					if (sIdList != '') {
						...
						var sURLData = APPRIS.EXPORT_DATA_SEQUENCE_PANEL['url']+'/id/'+sSpecie+'/'+sIdList+'?type=aa&format=fasta';
						var sURLData2 = APPRIS.EXPORT_DATA_SEQUENCE_PANEL['url2']+'/id/'+sSpecie+'/'+sIdList;
						sExportMenu =
							"<ul>\
							<div class='close' title='close'></div>\
							<div style='clear:both;'></div>\
...
// Open menu of agent data
function openAgentData(oEvent)
{
	$(this).parent().find('.agent').fadeIn(200, function(){
		if (!($(this).find('ul').length > 0)) {
			var sAgentMenu = '';
			var sFileName = '';
			var sTableData = '';
			...
			switch (sPanelType) {
				case 'sequence_panel':
					...
					if (iNumIdList == 0) {
						...
						sIdList = sIdList.replace(/,$/g,"");
						var sURLData = APPRIS.TOOL_DATA_SEQUENCE_PANEL['url']+'/id/'+sSpecie+'/'+sIdList+'?type=aa&format=fasta';
						sAgentMenu =
							"<ul>\
							<div class='close' title='close'></div>\
							<div style='clear:both;'></div>\
						...
						sIdList = sIdList.replace(/,$/g,"");
						var sURLData = APPRIS.TOOL_DATA_SEQUENCE_PANEL['url']+'/id/'+sSpecie+'/'+sIdList+'?type=aa&format=fasta';
						var sURLData2 = APPRIS.TOOL_DATA_SEQUENCE_PANEL['url2']+'/id/'+sSpecie+'/'+sIdList+'?type=aa&format=clw';
						sAgentMenu =
							"<ul>\
							<div class='close' title='close'></div>\
							<div style='clear:both;'></div>\
			...
			$(this).find('.agent_menu').html(sAgentMenu);
			$(this).find('.agent_link').bind('click',{
				filename: sFileName,
				table: sTableData
			}
			,popStatic);
...
function newWindow(event) {
	if ( $(this).attr('class') == 'pop_external_ref' && event.type == 'click' ) {
		...
		var sWProp = "agentbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no,"; 		

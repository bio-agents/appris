...
use Retrieve qw( get_sequence_result get_agent_result );
...
sub main()
{
	...
	print_http_response(400)
		unless ( defined $o_type and defined $q_type and defined $id_list );
	...
	# Get sequence result
	my ($sequences) = get_sequence_result($id_list, $type, 'fasta');

	# Get agent result
	my ($result) = get_agent_result($sequences, $o_type, $FORMAT);

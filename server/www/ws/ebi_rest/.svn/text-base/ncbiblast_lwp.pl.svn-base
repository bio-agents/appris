...
=item *
L<http://www.ebi.ac.uk/Agents/webservices/services/sss/ncbi_blast_rest>
...
$Id: ncbiblast_lwp.pl 2443 2013-01-25 10:33:57Z hpm $
...
my $baseUrl = 'http://www.ebi.ac.uk/Agents/services/rest/ncbiblast';
...
GetOptions(
	...
	'program|p=s'  => \$agent_params{'program'},   # blastp, blastn, blastx, etc.
	'database|D=s' => \$params{'database'},       # Database(s) to search
	'matrix|m=s'   => \$agent_params{'matrix'},    # Scoring martix to use
	'exp|E=f'      => \$agent_params{'exp'},       # E-value threshold
	'filter|f=s'   => \$agent_params{'filter'},    # Low complexity filter
	'align|A=i'    => \$agent_params{'align'},     # Pairwise alignment format
	'scores|s=i'   => \$agent_params{'scores'},    # Number of scores
	'alignments|n=i' => \$agent_params{'alignments'},   # Number of alignments
	'dropoff|d=i'    => \$agent_params{'dropoff'},      # Dropoff score
	'match_scores=s' => \$agent_params{'match_scores'}, # Match/missmatch scores
	'match|u=i'      => \$params{'match'},             # Match score
	'mismatch|v=i'   => \$params{'mismatch'},          # Mismatch score
	'gapopen|o=i'    => \$agent_params{'gapopen'},      # Open gap penalty
	'gapext|x=i'     => \$agent_params{'gapext'},       # Gap extension penality
	'gapalign|g'     => \$agent_params{'gapalign'},     # Optimise gap alignments
	'stype=s' => \$agent_params{'stype'},    # Sequence type
	'seqrange=s' => \$agent_params{'seqrange'},    # Query subsequence
...
sub load_params {
	...
	$agent_params{'database'} = \@dbList;

	# Match/missmatch
	if ( $params{'match'} && $params{'missmatch'} ) {
		$agent_params{'match_scores'} =
		  $params{'match'} . ',' . $params{'missmatch'};
	}
	
	# Compatability options, old command-line
	if(!$agent_params{'alignments'} && $params{'numal'}) {
		$agent_params{'alignments'} = $params{'numal'};
	}
	if(!$agent_params{'gapopen'} && $params{'opengap'}) {
		$agent_params{'gapopen'} = $params{'opengap'};
	}
	if(!$agent_params{'gapext'} && $params{'extendgap'}) {
		$agent_params{'gapext'} = $params{'extendgap'};
	}
...
Further information:
  ...
  http://www.ebi.ac.uk/Agents/webservices/services/sss/ncbi_blast_rest

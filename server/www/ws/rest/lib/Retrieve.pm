...
=head2 load_registry
  ...
  Example    : use Retrieve qw(load_registry);
...
sub get_method_result {
	...
	my ($analysis);
	my ($result);	
	my ($registry) = $self->load_registry($self->specie);	
...
=cut
sub get_agent_result {
	my ($self) = shift;
	my ($sequences, $type, $format) = @_;
	my ($result) = undef;

	my ($seq_tmpfile) = File::Temp->new( UNLINK => 0, SUFFIX => '.web_agent.appris' );
	...
	eval {
		...
		if ( defined $cmd_stdout and $cmd_stdout =~ /NCBI:([^\n]*)/ ) { # ncbi blast
			...
			my ($job_id) = $1;
			$result = "http://www.ebi.ac.uk/Agents/services/web_muscle/agentresult.ebi?jobId=$job_id"
		}
	};

...
=item *
L<http://www.ebi.ac.uk/Agents/webservices/services/msa/muscle_rest>
...
$Id: muscle_lwp.pl 2443 2013-01-25 10:33:57Z hpm $
...
my $baseUrl = 'http://www.ebi.ac.uk/Agents/services/rest/muscle';
...
GetOptions(
    ...
    'format|f=s'    => \$agent_params{'format'},    # Alignment format
    'tree|t=s'      => \$agent_params{'tree'},      # Tree
    'order|r=s'     => \$agent_params{'order'},     # Order of sequences in alignment
...
sub load_params {
	...
	# Compatiblity options (old command-line)
	if(!$agent_params{'format'} && $params{'output'}) {
		$agent_params{'format'} = $params{'output'};
	}
	if(!$agent_params{'tree'} && $params{'outputtree'}) {
		$agent_params{'tree'} = $params{'outputtree'};
	}
...
Further information:
  ...
  http://www.ebi.ac.uk/Agents/webservices/services/msa/muscle_rest

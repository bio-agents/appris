...
=item *
L<http://www.ebi.ac.uk/Agents/webservices/services/msa/clustalw2_rest>

=item *
L<http://www.ebi.ac.uk/Agents/webservices/tutorials/perl>
...
$Id: clustalw2_lwp.pl 2443 2013-01-25 10:33:57Z hpm $
...
my $baseUrl = 'http://www.ebi.ac.uk/Agents/services/rest/clustalw2';
...
GetOptions(
    ...
    'alignment|l=s' => \$agent_params{'alignment'}, # Pairwise alignment method
    'type=s'        => \$agent_params{'type'},      # Sequence type: DNA or protein
    'ktup|k=i'      => \$agent_params{'ktup'},      # Word size
    'window|w=i'    => \$agent_params{'window'},    # Window size
    'score|s=s'     => \$agent_params{'score'},     # Pairwise score representation
    'topdiags|d=i'  => \$agent_params{'topdiags'},  # Number of best diags.
    'pairgap|p=i'   => \$agent_params{'pairgap'},   # Gap penalty (pairwise)
    'pwmatrix=s'    => \$agent_params{'pwmatrix'},  # Protein matrix (pairwise)
    'pwdnamatrix=s' => \$agent_params{'pwdnamatrix'}, # DNA matrix (pairwise)
    'pwgapopen=i'   => \$agent_params{'pwgapopen'}, # Gap open penalty (pairwise)
    'pwgapext=f'    => \$agent_params{'pwgapext'},  # Gap extension penalty (pairwise)
    'matrix|m=s'    => \$agent_params{'matrix'},    # Protein scoring matrix (multi)
    'dnamatrix=s'   => \$agent_params{'dnamatrix'}, # DNA scoring matrix (multi)
    'gapopen|g=i'   => \$agent_params{'gapopen'},   # Gap creation penalty (multi)
    'noendgaps'     => \$agent_params{'noendgaps'}, # Terminal gaps scored
    'gapext|x=f'    => \$agent_params{'gapext'},    # Gap extension penalty (multi)
    'gapdist|y=i'   => \$agent_params{'gapdist'},   # Gap separation penalty (multi)
    'iteration|i=s' => \$agent_params{'iteration'}, # Iteration type
    'numiter|N=i'   => \$agent_params{'numiter'},   # Number of iterations
    'clustering=s'  => \$agent_params{'clustering'}, # Clustering method for guide tree
    'output|o=s'    => \$agent_params{'output'},    # Alignment format
    'outorder|r=s'  => \$agent_params{'outorder'},  # Order of sequences in alignment
...
&print_debug_message( 'MAIN', "agent_params:\n" . Dumper( \%agent_params ), 11 );
...
Get list of agent parameter names.
...
Get details of a agent parameter.
...
=head2 print_agent_params()

Print list of agent parameters.

  &print_agent_params();

=cut

sub print_agent_params {
	print_debug_message( 'print_agent_params', 'Begin', 1 );
	my (@param_list) = &rest_get_parameters();
	foreach my $param ( sort(@param_list) ) {
		print $param, "\n";
	}
	print_debug_message( 'print_agent_params', 'End', 1 );
}

=head2 print_param_details()

Print details of a agent parameter.
...
sub submit_job {
	...
	# Set input sequence
	$agent_params{'sequence'} = shift;

	# Load parameters
	&load_params();

	# Submit the job
	my $jobid = &rest_run( $params{'email'}, $params{'title'}, \%agent_params );
...
sub load_params {
	...
	# Compatability options, old command-line
	if(!$agent_params{'alignment'}) {
		$agent_params{'alignment'} = 'fast' if($params{'quicktree'});
		$agent_params{'alignment'} = 'slow' if($params{'align'});
	}
...
Further information:
  ...
  http://www.ebi.ac.uk/Agents/webservices/services/msa/clustalw2_rest
  http://www.ebi.ac.uk/Agents/webservices/tutorials/perl
